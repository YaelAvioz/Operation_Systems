...
void compile(char *file_name, char *user_name, char *input, char *output, int results_file)
{
    int pid1 = fork();
    ...
    if (pid1 == 0)
    {
        char *args[] = {"gcc", "-o", "compiled.out", file_name, NULL};
        execvp(args[0], args);
        exit(-1);
    }
...
int main(int argc, char *argv[])
{
    ...
    int errors_file = open("errors.txt", O_CREAT | O_WRONLY, 0666);
    ...
    for (; entry = readdir(dir_fd); entry != NULL)
    {
        ...
        memset(full_path, 0, MAXLEN);
        snprintf(full_path, MAXLEN, "%s/%s", dir, entry->d_name);

        DIR *subdir_fd;
        subdir_fd = opendir(full_path);
        ...
        for (; subdir_entry = readdir(subdir_fd); subdir_entry != NULL)
        {
            if (subdir_entry->d_type == DT_REG || subdir_entry->d_type == DT_UNKNOWN){
                ...
                const char *dot = strrchr(subdir_entry->d_name, '.');
                if ((!(!dot || dot == subdir_entry->d_name)) && (strcmp(dot + 1, "c") == 0)){
                    char file_name[MAXLEN];
                    memset(file_name, 0, MAXLEN);
                    snprintf(file_name, MAXLEN, "%s/%s", full_path, entry->d_name);
                    compile(file_name, entry->d_name, input, output, results_file);
                    is_found = 1;
                    break;
                }
            }
